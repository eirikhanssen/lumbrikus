  ...
  <xsl:template match="/">
    <ol class="icons">
      <xsl:for-each select="//svg:symbol">
        <li>
          <svg><xsl:attribute name="class"><xsl:text>icon</xsl:text></xsl:attribute><use><xsl:attribute name="xlink:href">/media/svg/lumbrikus-symbol-icons.svg#<xsl:value-of select="@id"/></xsl:attribute></use></svg>
          <a>
